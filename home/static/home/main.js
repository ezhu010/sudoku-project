...
window.onload = function () {
  ...
  document
    .getElementById("showSolution-btn")
    .addEventListener("click", showSolution);
  document
    .getElementById("checkSolution-btn")
...
function generateBoard(time) {
  ...
  if (timer) {
    document
      .getElementById("timer-numbers")
      .removeAttribute("class", "red_timer");
    ...
    if (totalTime <= -1001) {
      //totalTime is -1000 when it is manually set to that
      totalTime = time;
      runTimer();
    } else {
      //don't want to run timer again if it is already running
      totalTime = time;
    }
  }
  var counter = 0;
  ...
  for (let i = 0; i < 9; i++) {
    ...
    for (let j = 0; j < 9; j++) {
      ...
      if (board[i][j] != 0) {
        ...
        box.classList.add("boxes");
        box.innerHTML =
          '<input  type="text" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength = "1" onkeypress="return isNumberKey(event)" class="grid_box_input" min="1" max="9" >';
        newRow.appendChild(box);
      }
      if (i == 2 || i == 5) {
...
function runTimer() {
  var refreshID = setInterval(function () {
    ...
    if (totalTime <= 1000 * 15) {
      document
        .getElementById("timer-numbers")
        .setAttribute("class", "red-timer-flash");
    } else if (totalTime <= 1000 * 30) {
      document
        .getElementById("timer-numbers")
        .setAttribute("class", "red-timer");
    }
    if (seconds >= 10) {
...
function checkSolution() {
  ...
  for (var i = 0; i < 81; i++) {
    ...
    if (val == "") {
      ...
      document
        .getElementById(i)
        .firstElementChild.setAttribute("class", "green_grid_box_input");
    } else if (val != solvedBoard[row][col]) {
      document
        .getElementById(i)
        .firstElementChild.setAttribute("class", "red_grid_box_input");
      flag = false;
    }
  }
  if (flag) {
